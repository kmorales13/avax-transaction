import { Address, parseEther, encodeFunctionData, parseUnits } from "viem"
...
export function useTransaction() {
  ...
  async function send(to: Address, amount: string, tokenAddress: Address | undefined) {
    // prepare transaction config based on token type
    const transactionConfig = tokenAddress ? {
      to: tokenAddress,
      data: encodeFunctionData({ // hardcoded for USDC
        abi: USDC_TOKEN_ABI,
